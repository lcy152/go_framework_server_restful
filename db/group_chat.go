...
func (database *Database) DeleteGroupChat(ctx context.Context, guid string) error {
	db := database.DB.Collection(table.GroupChat)
...
func (database *Database) GetGroupChat(ctx context.Context, guid string) *model.ChatMessage {
	db := database.DB.Collection(table.GroupChat)
